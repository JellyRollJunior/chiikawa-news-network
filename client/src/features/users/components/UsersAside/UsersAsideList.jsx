      ...
      <ul className="flex flex-1 flex-col gap-2">
        {[...Array(14)].map((item, index) => (
          <li className="flex gap-2">
            <div className={`px-2 pt-2 pb-1.5 ${getBlockStyle(index)}`}>
              <LoadingElement className="size-[54px] rounded-full" />
            </div>
            <div
              className={`flex flex-1 items-center px-2 py-2 ${getBlockStyle(index)} `}
            >
    ...
    <ul className="flex flex-1 flex-col gap-2">
      {users.map((user, index) => (
        ...
        >
          ...
          <div
            className={`flex flex-1 items-center justify-center truncate px-2 py-2 lg:justify-start ${getBlockStyle(index)}`}
          >
